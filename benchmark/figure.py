...
def draw(CSV_File):
    ...
    for i in range(0, row_len):
        hashmap.setdefault(re.findall("conv-(\d+)-", Data[i][0])[0], []).append(
            Data[i]
        )
    ...
    for i in hashmap:
        ...
        for data in hashmap[i]:
            ...
            sub.x.append(
                float(data[1])
            )  # 将第一列数据从第二行读取到最后一行赋给列表x
            sub.y.append(
                float(data[2])
            )  # 将第二列数据从第二行读取到最后一行赋给列表y
        plotList.append(sub)
    for i in range(1, len(plotList) + 1):
        ...
        plt.barh(
            idx + width, width=plotList[i - 1].y, height=width, label="default"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-STRIP", type=int, help="Set the size of each vector slice, default=32"
    )
    parser.add_argument(
        "-FILTER_min",
        type=int,
        help="Set the MINIMUM size of the convolution kernel, default=3",
    )
    parser.add_argument(
        "-FILTER_max",
        type=int,
        help="Set the MAXIMUM size of the convolution kernel, default=11",
    )
    parser.add_argument(
        "-FILTER_step",
        type=int,
        help="Set the step size of the convolution kernel size change, default=2",
    )
    parser.add_argument(
        "-OUTPUT_min",
        type=int,
        help="Set the MINIMUM size of the output matrix, default=32",
    )
    parser.add_argument(
        "-OUTPUT_max",
        type=int,
        help="Set the MAXIMUM size of the output matrix, default=1024",
    )
    parser.add_argument(
        "-OUTPUT_step",
        type=int,
        help="Set the step size of the output matrix size change(OUTPUT+=step), default is OUTPUT*=2",
    )
    parser.add_argument(
        "-CSV", help="Generate figure directly using the given csv file"
    )
    parser.add_argument(
        "-CONV_OPT",
        help="the path of the conv-opt (MLIR convolution optimizer with CB-SM approach)",
    )
    parser.add_argument(
        "-MLIR_OPT",
        help="the path of the mlir-opt (MLIR modular optimizer driver).",
    )
    parser.add_argument(
        "-PASS",
        type=int,
        help="the number of test passes used to average the results.",
    )
