...
using BodyBuilderFn =
    mlir::function_ref<void(OpBuilder &, Location, ValueRange)>;

void OnDeviceOp::build(OpBuilder &builder, OperationState &result,
                       llvm::StringRef targetId, llvm::StringRef targetConfig,
                       TypeRange resultTypes, ValueRange args,
                       BodyBuilderFn bodyBuilder, Type asyncTokenType,
                       ValueRange asyncDependencies) {
  result.addAttribute(getTargetIdAttrName(result.name),
                      builder.getStringAttr(targetId));
  result.addAttribute(getTargetConfigAttrName(result.name),
                      builder.getStringAttr(targetConfig));
...
void OnDeviceOp::print(OpAsmPrinter &p) {
  ...
  p << ' ';
  p.printFunctionalType(getInnerOperands().getTypes(),
                        getInnerResults().getTypes());
  p << ' ';
  p.printRegion(getRegion());
  elidedAttrs.push_back("operand_segment_sizes");
}

// operation ::= `sche.on_device` (`async` `[` ssa-id-list `]`)? `(`$operands`)`
// attr-dict functional-type(operands, results) $region
::mlir::ParseResult OnDeviceOp::parse(::mlir::OpAsmParser &parser,
  ...
  std::unique_ptr<::mlir::Region> regionRegion =
  ...
  result.addAttribute(
      ...
      parser.getBuilder().getDenseI32ArrayAttr(
          {static_cast<int32_t>(asyncDependencies.size()),
           static_cast<int32_t>(innerOperandsOperands.size())}));
  result.addAttribute("result_segment_sizes",
                      parser.getBuilder().getDenseI32ArrayAttr(
                          {static_cast<int32_t>(parser.getNumResults()) -
                               static_cast<int32_t>(innerResultsTypes.size()),
                           static_cast<int32_t>(innerResultsTypes.size())}));
  result.addTypes(innerResultsTypes);
  if (parser.resolveOperands(innerOperandsOperands, innerOperandsTypes,
                             innerOperandsOperandsLoc, result.operands))
    return ::mlir::failure();
  return ::mlir::success();
}
